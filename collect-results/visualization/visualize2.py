...
from visualization.logger import read_json, read_csv
...
def plot_results(
    ...
    if tiling == 'vertical' or tiling is None:
        ...
        nrows = N // ncols
    figsize = figsize or (6.2 * ncols, 6.2 * nrows)
    ...
    for (isplit, sk) in enumerate(sorted(sk2r.keys())):
        ...
        if any(g2l.keys()):
            ax.legend(
                g2l.values(),
                ['%s '%(g+'_core') for g in g2l] if average_group else g2l.keys(),
                loc='bottom right' if legend_outside else None,
...
LOG_DIRS = '../together/pong/CNN_RNN_Series_8-0'
results = load_results(LOG_DIRS)
fig = plot_results(results, average_group=False,
                      split_fn=lambda _:
'',
                      shaded_std=False,shaded_err=True,
